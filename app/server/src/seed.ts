...
function main(): void {
  ...
  ).run(adminId, 'InkedUp Admin', adminEmail, '+6281234567890', bcrypt.hashSync(adminPassword, 10), 'admin');

  // Demo customer
  const customerId = generateId('user');
  db.prepare(
    'INSERT INTO users (id, name, email, phone, password_hash, role) VALUES (?, ?, ?, ?, ?, ?)'
  ).run(customerId, 'Demo Customer', 'customer@example.com', '+6281234567899', bcrypt.hashSync('customer123', 10), 'customer');
  ...
  for (const loc of locations) {
    db.prepare('INSERT INTO locations (id, name, slug, zone, call_out_fee, priority, description) VALUES (?, ?, ?, ?, ?, ?, ?)').run(
  ...
  for (const style of styles) {
    db.prepare('INSERT INTO styles (id, name, slug, display_order) VALUES (?, ?, ?, ?)').run(
  ...
  for (const studio of mockStudios) {
    db.prepare(
      'INSERT INTO studios (id, name, email, whatsapp_number, location, address, status, artist_ids, instagram, bio) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(
      studio.id,
      studio.name,
      studio.email,
  ...
  for (const page of contentPages) {
    db.prepare('INSERT INTO content_pages (id, slug, title, page_type, body) VALUES (?, ?, ?, ?, ?)').run(
  ...
  for (const r of reviewComments) {
    db.prepare(
      'INSERT INTO reviews (id, artist_id, artistry, professionalism, cleanliness, communication, value, comment, is_published) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(
      generateId('review'),
      r.artistId,
      r.rating,
  ...
  for (const artist of mockArtists) {
    db.prepare('UPDATE artists SET rating = ?, review_count = ? WHERE id = ?').run(artist.rating, artist.reviewCount, artist.id);
  }

  console.log('Database seeded successfully.');
  console.log('Admin login: admin@inkedup.id / admin123');
